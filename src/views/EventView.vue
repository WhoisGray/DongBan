...
const event = computed(() => store.findEvent(props.id)); const tab = ref('overview'); const showExpense = ref(false); const showPeople = ref(false); const showDetails = ref(false); const showTransfer = ref(false)
...
<template>
  <div v-if="event" class="page stack-lg">
    <section class="event-banner">
      ...
      <div class="event-banner__main"><div><span class="eyebrow">{{ formatPersianDate(event.date, { dateStyle: 'long' }) }}</span><h1>{{ event.title }}</h1><p>{{ event.note || 'برای این رویداد توضیحی ثبت نشده است.' }}</p></div><div class="event-banner__actions"><button class="btn btn--ghost" @click="showTransfer = true">⇄ انتقال داده</button><button class="btn btn--ghost" @click="showDetails = true">تنظیمات رویداد</button><button class="btn btn--ghost" @click="showPeople = true">＋ اعضا</button><button class="btn btn--primary" :disabled="event.people.length < 2" @click="showExpense = true">＋ ثبت هزینه</button></div></div>
    ...
    <BaseModal v-if="showTransfer" title="انتقال و پشتیبان رویداد" @close="showTransfer = false"><EventTransferPanel :event="event" @done="showTransfer = false" /></BaseModal>
