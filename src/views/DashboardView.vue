...
import { formatPersianDate, todayIso } from '../utils/date.js'
...
const events = computed(() =>
  store.state.events.filter((event) =>
    event.title.toLocaleLowerCase('fa').includes(query.value.toLocaleLowerCase('fa'))
  )
)
...
function duplicate(event) {
  const copy = store.duplicateEvent(event.id)
  if (copy) router.push(`/event/${copy.id}`)
}

function remove(event) {
  if (window.confirm(`رویداد «${event.title}» و تمام هزینه‌هایش حذف شود؟`)) {
  ...
  <div class="page stack-lg">
    ...
    <section>
      <div class="section-heading dashboard-heading">
        <div>
          <span class="eyebrow">آرشیو شخصی</span>
          <h2>رویدادهای من</h2>
        </div>
        <input
      ...
      <div v-if="events.length" class="event-grid">
        <TransitionGroup name="card-anim">
          ...
          >
            <header>
              <span class="event-card__icon transition-transform duration-200 group-hover:scale-105">✦</span>
              <div class="row-actions" @click.stop>
                <button class="icon-btn" title="ساخت کپی بدون هزینه‌ها" @click="duplicate(event)">
                  ⧉
                </button>
                <button class="icon-btn icon-btn--danger" title="حذف" @click="remove(event)">
                  ×
                </button>
              </div>
            </header>
            ...
            <div class="my-4">
              <small class="text-xs text-muted">{{ formatPersianDate(event.date) }}</small>
              <h3 class="font-bold text-ink transition-colors group-hover:text-primary">
                {{ event.title }}
              </h3>
              <p class="line-clamp-2 text-xs text-muted">
                {{ event.note || 'بدون توضیح' }}
              </p>
            </div>

            <footer>
              <strong>{{ formatMoney(total(event), event.currency) }}</strong>
              <span>{{ event.people.length }} نفر · {{ event.expenses.length }} هزینه</span>
            </footer>
          </article>
        </TransitionGroup>
      </div>
      ...
      <div v-else class="empty empty--large">
        ...
        <h2>{{ store.state.events.length ? 'رویدادی پیدا نشد' : 'هنوز رویدادی نداری' }}</h2>
        <p>سفر، مهمانی یا خانهٔ مشترک؛ اولین رویداد را بساز.</p>
        <button v-if="!store.state.events.length" class="btn btn--primary" @click="showCreate = true">
          ساخت اولین رویداد
        </button>
      </div>
    </section>
